...
export default class NotifyMixin extends Vue {
  ...
  public getErrorMessageFromResponse(error: any): any {
    const paramLength: number = error.response.headers['x-xeremingressosapp-param-length'];
    if (!paramLength || paramLength == 0) {
      return this.$t(error.response.headers['x-xeremingressosapp-error']);
    }

    if (paramLength == 1) {
      return this.$t(error.response.headers['x-xeremingressosapp-error'], {
        param: decodeURIComponent(error.response.headers['x-xeremingressosapp-param'].replace(/\+/g, ' ')),
      });
    }

    const params = {};
    for (let i = 0; i < paramLength; i++) {
      params['param' + i] = decodeURIComponent(error.response.headers['x-xeremingressosapp-param' + i].replace(/\+/g, ' '));
    }
    return this.$t(error.response.headers['x-xeremingressosapp-error'], params);
  }

  public getAlertMessageFromResponse(response: any): any {
    return this.$t(response.headers['x-xeremingressosapp-alert'], {
      param: decodeURIComponent(response.headers['x-xeremingressosapp-params'].replace(/\+/g, ' ')),
    });
  }
}
